  ...
  return (
    <Card className="shadow-md border-slate-200/70 dark:border-slate-800">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          ...
          <div className="flex flex-col items-end gap-1">
            <Badge className="bg-teal-500 hover:bg-teal-600">AI Summary</Badge>
                ...
                {keyTopics.map((topic, index) => (
                  <div key={index} className="flex items-center justify-between rounded-lg bg-slate-50 dark:bg-slate-800/50 p-2">
                    <div className="flex items-center gap-2">
                      <div className="flex h-6 w-6 items-center justify-center rounded-full bg-teal-100 text-teal-700 dark:bg-teal-900 dark:text-teal-300">
                        {index + 1}
                      </div>
                      <div>
                ...
                {decisions.map((decision, index) => (
                  <div key={index} className="rounded-lg border border-slate-200 dark:border-slate-800 p-2.5">
                    <div className="flex items-start gap-2">
                      <svg className="h-4 w-4 mt-0.5 text-teal-500" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <div className="space-y-2">
                {insights.map((insight, index) => (
                  <div key={index} className="flex items-start gap-2 rounded-lg bg-slate-50 dark:bg-slate-800/50 p-2.5">
                    {insight.icon || (
                      <svg className={`h-4 w-4 mt-0.5 ${insight.color || 'text-teal-500'}`} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <CardFooter className="flex justify-between border-t border-slate-100 dark:border-slate-800 pt-4">
        <button className="text-sm text-slate-500 hover:text-slate-900 dark:hover:text-slate-100 transition-colors">
          Export to PDF
        </button>
        <button className="text-sm text-teal-600 hover:text-teal-700 dark:text-teal-400 dark:hover:text-teal-300 transition-colors">
          Share Summary
        </button>
      </CardFooter>
    </Card>
